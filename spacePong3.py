...
TARGET_WIDTH = 30
TARGET_HEIGHT = 30
TARGET_SPAWN_INTERVAL = 2000  # In milliseconds (2 seconds)
...
pygame.time.set_timer(pygame.USEREVENT, 1000)  # Timer event every 1 second
...
def game_over_screen():
    ...
    window.blit(game_over_text, (WINDOW_WIDTH // 2 - 80, WINDOW_HEIGHT // 2 - 30))
    window.blit(score_text, (WINDOW_WIDTH // 2 - 80, WINDOW_HEIGHT // 2 + 10))
    pygame.display.update()
    pygame.time.delay(3000)  # Display the game-over screen for 3 seconds before closing
    pygame.quit()
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.USEREVENT:
            game_time -= 1
            if game_time <= 0:
                game_time = 0  # Prevent timer from going negative
    ...
    # Spawn new targets at regular intervals
    if random.random() < 0.02:
        spawn_target()

    # Move the targets downwards
    ...
    window.blit(time_text, (WINDOW_WIDTH - 150, 20))
